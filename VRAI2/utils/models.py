...
from VRAI2.utils.preprocessing import SNV, SavitzkyGolaySmooth, MSC
from VRAI2.utils.visual import plot_predictions, plot_cm
...
def compute_regression_metrics(y_true, y_pred):
    ...
    # std/SEP
    std_SEP = std_pred / sep
    
    return {
        "SEP": sep, "std_SEP": std_SEP, "Bias": bias, "SEPC": sepc, "R2": r2, "Slope": slope, "N": n,
...
def train_xgb_class(X_train, X_val, y_train, y_val, outpath, beams_step=1):
    ...
    # ===== Validation metrics =====
    preds = final_class.predict(X_val)
    ...
    train_dict[f"train_Accuracy"] = (final_class.predict(X_train) == y_train_enc).mean()
    ...
    # ===== Training metrics =====
    train_preds = final_class.predict(X_train)
...
def train_svm_regr(X_train, X_val, y_train, y_val, y_col, outpath, beams_step=1):
    ...
    return {
        "model": final_reg.best_estimator_,
        "selected_indices": selected_indices
    }
...
def train_svm_classif(X_train, X_val, y_train, y_val, y_col, outpath, beams_step=1):
    ...
    # Plot confusion matrix
    plot_cm(confusion_matrix(y_val_enc, pred), model='SVM', classes=class_names, acc=accuracy, outpath=outpath)
...
def train_mlp_regr(X_train, X_val, y_train, y_val, y_col, outpath, beams_step=1):
    ...
    print(f"Numero bande selezionate da MLP: {num_bands_selected}")
    print("Indici bande selezionate:", selected_indices)
    print("Best params:", reg.best_params_)
    ...
    # ===== Save CSV =====
    df_output = pd.DataFrame([train_dict])
    outname = f"{y_col}_regr_MLP_{'each' + str(beams_step) if beams_step > 1 else ''}.csv"
