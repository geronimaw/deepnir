...
if __name__ == "__main__":
    ...
    for y_col in y_train_all[list(y_train_all.keys())[0]].keys():
        ...
        # Train for regression and classification (on dataset labels)
        print(f"\nTraining combined XGBoost model for target: {y_col}")
        results = train_xgb_regr(X_train_combined, X_val_combined, y_train_combined, y_val_combined, 
        ...
        if y_col == "DM" or y_col == "SS": 
            ...
            results = train_xgb_class(X_train_combined, X_val_combined, train_dirs, val_dirs,
