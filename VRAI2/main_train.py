...
from VRAI2.utils.models import train_xgb_regr, train_xgb_class, train_svm_regr, train_svm_classif
...
def build_output_dirs(base_out: Path, freq_tag: str, sub: str) -> tuple[Path, Path]:
    """Return (csv_dir, models_dir) and create them if they don't exist."""
    csv_dir    = base_out / "outputs_std_PEC" / freq_tag / sub
    models_dir = base_out / "trained_models"  / freq_tag / sub
...
def train_multi_dataset(
    ...
    csv_dir, models_dir = build_output_dirs(OUT_PATH, freq_tag, sub)
...
def train_per_family(
    ...
    for dataset_name in X_train_all:
        ...
        csv_dir, models_dir = build_output_dirs(
            OUT_PATH, freq_tag, dataset_name
        )
