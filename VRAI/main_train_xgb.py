...
if __name__ == "__main__":
    ...
    output_csv = os.path.join(out_path, "outputs", "multi_dataset_training_xgb")
        ...
        for file in os.listdir(os.path.join(in_path, dir)):
            if file.endswith("DATASET.xlsx"):
                ...
                X_train_all[dataset_name], X_val_all[dataset_name], y_train_all[dataset_name], y_val_all[dataset_name] = main(
                    dir, sheet_names, os.path.join(in_path, dir, file), args.beams_step)
    ...
    for y_col in y_train_all[list(y_train_all.keys())[0]].keys():
        ...
        # train_xgb_regr(X_train_combined, X_val_combined, y_train_combined, y_val_combined, 
        #                 y_col, output_csv, args.beams_step)

        # Train classification model for dataset labels
        if y_col == "DM" or y_col == "SS": 
            print("\nTraining combined XGBoost classification model")
            train_xgb_class(X_train_combined, X_val_combined, train_dirs, val_dirs,
